...
<style>
 ...
 @media (max-width:700px){
  ...
  table.w{font-size:12px} table.w td,table.w th{padding:6px 8px}
  #tip{max-width:80vw}
 }
...
<div id="boot" style="background:#7a1f1f;color:#fff;padding:8px 14px;font:13px monospace;white-space:pre-wrap">Dashboard-Skript wird geladen … (bleibt diese Zeile stehen, ist das Skript nicht gestartet)</div>
<script>
...
async function loadChart(){
 ...
 try{LAST_CHART=await getJSON(`/chart?coin=${coin}&agg=${agg}&minutes=${mins}&fvg=${fvg}`);
  computeSignal();
...
async function load(){
 ...
 const d=await getJSON(`/data?coin=${coin}&hours=${h}`);
 LAST=d.grid; drawHeat(d.grid); drawDepth(d.depth);
 try{LAST_WHALES=await getJSON(`/whales?coin=${coin}&hours=${Math.max(72,h)}`); drawWhales(LAST_WHALES); computeSignal();
