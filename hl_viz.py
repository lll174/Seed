...
<style>
 table.w{border-collapse:collapse;width:100%;font-size:11px}
 table.w th{color:#8b949e;font-weight:500;text-align:left;padding:4px 8px;
   border-bottom:1px solid #21262d;font-size:10px;text-transform:uppercase}
 table.w td{padding:5px 8px;border-bottom:1px solid #161b22;white-space:nowrap}
 table.w td.n{text-align:right;font-variant-numeric:tabular-nums}
...
let LAST=null, LAST_CHART=null, GEO={};
...
function drawChart(d){
 ...
 GEO.chart={C,L,Lm,T,pw,ph,cw,lo,hi,H,B,R,W};
 ...
 $('chartinfo').textContent=`${C.length} Kerzen · ${L.length} Liquidationsmarken`
  +` · Stundenvolumen ${fmt(d.vol_1h||0)} USD`;
}
...
function drawWhaleTable(d){
 ...
 $('whaletable').innerHTML=C.length?`<table class="w"><tr>
  <th>Wallet</th><th>Coin</th><th>Seite</th><th>Wert</th><th>Hebel</th><th>Einstieg</th>
  <th>Liq.-Preis</th><th>Abstand</th><th>unreal. PnL</th><th>offen seit</th><th>Stand</th></tr>`
...
async function loadChart(){
 ...
 try{const r=await fetch(`/chart?coin=${coin}&agg=${agg}&minutes=${mins}`);
  drawChart(await r.json());}catch(e){}
}
async function load(){
 loadChart();
