...
def collect_hwrng(mock: bool) -> SourceResult:
    if mock:
    ...
    return SourceResult("BCM2712 HWRNG", dsha512("hwrng", raw), len(raw),
...
def collect_decay(n_events: int, mock: bool) -> SourceResult:
    timestamps = []
    ...
    if mock:
        ...
        for i in range(n_events):
            ...
            timestamps.append(t)
            if i % max(1, n_events // 50) == 0 or i == n_events - 1:
    ...
    return SourceResult("Radiozerfall (CAJOE)", dsha512("decay", raw), len(raw),
...
def collect_camera(n_frames: int, mock: bool) -> SourceResult:
    frames = []
    if mock:
        for i in range(n_frames):
            frames.append(os.urandom(832 * 2))
        ...
        try:
            for i in range(n_frames):
                ...
                bus.i2c_rdwr(wmsg)
                frames.append(frame)
                progress("IR-Kamera", i + 1, n_frames)
        except OSError as e:
    ...
    raw = b"".join(frames)
    return SourceResult("MLX90640 IR-Kamera", dsha512("cam", raw), len(raw),
                        credited_bits=n_frames * CAM_ENTROPY_PER_FRAME,
                        info=f"{n_frames} Roh-Frames ({len(raw)} B)")
...
def collect_dice(n_rolls: int, target_bits: int, mock: bool) -> SourceResult:
    rolls = []
    if mock:
        import random
    ...
    return SourceResult("Wuerfel (manuell)", dsha512("dice", raw), len(raw),
                        credited_bits=bits,
...
def combine(hw: SourceResult, dec: SourceResult, cam: SourceResult,
            dice: SourceResult, n_bytes: int) -> bytes:
...
def configure(args):
    if args.yes:
        return args.words, args.decay, args.frames, args.dice
    ...
    decay  = ask_int(f"Zerfallsereignisse (Empf. ≥{max(512, 2*target)}; "
                     f"~25 CPM Hintergrund ⇒ {max(512,2*target)/25:.0f} min)",
                     args.decay, 64, 100000)
    frames = ask_int("Kamera-Frames MLX90640 (Empf. ≥16)", args.frames, 2, 1000)
    dice   = ask_int(f"Wuerfelwuerfe (Empf. ≥{rec_dice} fuer volle Unabhaengigkeit)",
                     args.dice, 10, 1000)
    return words, decay, frames, dice
...
def main():
    ...
    n_words, n_decay, n_frames, n_dice = configure(args)
    ...
    r = collect_hwrng(args.mock); results.append(r)
    ...
    r = collect_decay(n_decay, args.mock); results.append(r)
    status_line(2, 4, r.name, "ok", r.info)

    print(C.BOLD + "\n Phase 3/4 — MLX90640 IR-Kamera" + C.RESET)
    r = collect_camera(n_frames, args.mock); results.append(r)
    status_line(3, 4, r.name, "ok", r.info)

    print(C.BOLD + "\n Phase 4/4 — Wuerfel (hardwareunabhaengige letzte Quelle)" + C.RESET)
    r = collect_dice(n_dice, target_bits, args.mock); results.append(r)
    ...
    print(C.BOLD + "\n Kombiniere: FINAL = HMAC-SHA512(key=H(Wuerfel), msg=Pool(HWRNG,Zerfall,Kamera,OS))"
          + C.RESET)
    entropy = combine(hw, dec, cam, dice, n_bytes)
