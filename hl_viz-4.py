...
function computeSignal(){
 ...
 if(fb) add('buy',1,`Offenes bullisches FVG ${px(fb.bot)}–${px(fb.top)} direkt unter dem Kurs`);
 if(fs) add('sell',1,`Offenes bärisches FVG ${px(fs.bot)}–${px(fs.top)} direkt über dem Kurs`);
 const inb=F.filter(f=>f.kind==='bull'&&px0>=f.bot&&px0<=f.top)[0], ins=F.filter(f=>f.kind==='bear'&&px0>=f.bot&&px0<=f.top)[0];
 if(inb) add('buy',1,`Kurs steht IM bullischen FVG ${px(inb.bot)}–${px(inb.top)}`);
 if(ins) add('sell',1,`Kurs steht IM bärischen FVG ${px(ins.bot)}–${px(ins.top)}`);
 ...
 // 7. Danger-Time aus der Zeit-Heatmap: keine Richtung, nur Warnung
 ...
 box.innerHTML=`<div style="font-size:15px;font-weight:600;color:${color};margin-bottom:6px">${label}
  ...
  +`Orderbuch ab 30 % Imbalance, Funding ab −0,005 / +0,01 %/h, Wale ab 1,5-fachem Übergewicht.</div>`;
}
...
async function load(){
 ...
 try{const rw=await fetch(`/whales?coin=${coin}&hours=${Math.max(72,h)}`);
  LAST_WHALES=await rw.json(); drawWhales(LAST_WHALES); computeSignal();}catch(e){}
