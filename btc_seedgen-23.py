...
def security_briefing(args):
    ...
    zeile(t("   bleiben — oder die SD-Karte wird geloescht/neu beschrieben.",
            "   generation — or the SD card must be erased/re-imaged."))
    ...
    try:
        antwort = input(C.ORA + t(
            "   Hast du diese Regeln verstanden und haeltst du sie ein? [ja/nein]: ",
            "   Have you understood these rules and will you follow them? [yes/no]: ")
...
def disable_wifi(args):
    ...
    if methode:
        ...
        print(C.DIM + t("   Bleibt auch nach Reboot aus.", "   Stays off across reboots.") + C.RESET)
    else:
        if args.mock:
...
# ----------------------------------------------------------------------------
def offer_permanent_radio_off(args):
    """Bietet an, WLAN+Bluetooth per dtoverlay in config.txt dauerhaft zu
    deaktivieren: Die Funkchips werden dann beim Boot gar nicht mehr
    initialisiert — haerter als der rfkill-Soft-Block. Wirkt ab dem
    naechsten Reboot; rueckgaengig nur durch Entfernen der Zeilen."""
    if args.yes or args.mock:
        return
    pfad = None
    for p in ("/boot/firmware/config.txt", "/boot/config.txt"):
        if os.path.exists(p):
            ...
            break
    if pfad is None:
        print(C.YEL + t("   Hinweis: config.txt nicht gefunden — dauerhafte Firmware-"
                        "Abschaltung manuell: dtoverlay=disable-wifi / disable-bt",
                        "   Note: config.txt not found — permanent firmware disable "
                        "manually: dtoverlay=disable-wifi / disable-bt") + C.RESET)
        return
    try:
    ...
    if hat_wifi and hat_bt:
        print(C.GRN + t(" ✔ Funkchips sind bereits auf Firmware-Ebene deaktiviert "
                        ...
                        " ✔ Radio chips are already disabled at firmware level "
                        "(dtoverlay in config.txt).") + C.RESET)
        return
    print(C.DIM + t("   Optional: WLAN+Bluetooth DAUERHAFT auf Firmware-Ebene abschalten —",
                    "   Optional: disable WLAN+Bluetooth PERMANENTLY at firmware level —") + C.RESET)
    print(C.DIM + t("   die Chips werden dann beim Boot gar nicht mehr initialisiert.",
                    "   the chips will then not be initialized at boot at all.") + C.RESET)
    try:
        antwort = input(C.ORA + t(
            "   Dauerhaft deaktivieren (dtoverlay in config.txt eintragen)? [ja/nein]: ",
            "   Disable permanently (add dtoverlay to config.txt)? [yes/no]: ")
            + C.RESET + C.GRN).strip().lower()
        sys.stdout.write(C.RESET); sys.stdout.flush()
    except (EOFError, KeyboardInterrupt):
        sys.stdout.write(C.RESET)
        return
    if antwort not in ("ja", "yes", "y"):
        print(C.DIM + t("   Uebersprungen — rfkill-Block bleibt aktiv.",
                        "   Skipped — rfkill block remains active.") + C.RESET)
        return
    try:
        ...
        with open(pfad, "a") as f:
            f.write(zusatz)
        print(C.GRN + t(f" ✔ Eingetragen in {pfad} — wirksam ab dem naechsten Reboot.",
                        f" ✔ Written to {pfad} — takes effect after the next reboot.") + C.RESET)
        print(C.DIM + t("   Rueckgaengig: die dtoverlay-Zeilen dort wieder entfernen.",
                        "   To undo: remove those dtoverlay lines again.") + C.RESET)
    except Exception as e:
        print(C.YEL + t(f"   ⚠ Eintrag fehlgeschlagen ({e}) — rfkill-Block bleibt aktiv.",
                        f"   ⚠ Write failed ({e}) — rfkill block remains active.") + C.RESET)
...
def main():
    ...
    disable_wifi(args)
    offer_permanent_radio_off(args)
