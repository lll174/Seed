...
def disable_wifi(args):
    ...
    if getattr(args, "keep_wifi", False):
        print(C.YEL + " ⚠ Funkmodule (WLAN+Bluetooth) bleiben auf Wunsch AKTIV "
              "(--keep-wifi) — fuer echte Seeds nicht empfohlen!" + C.RESET)
        return
    import subprocess
    methode = None
    try:
    ...
    if methode:
        ...
        print(C.GRN + f" ✔ WLAN + Bluetooth deaktiviert via {methode}{status}. "
              "Ethernet bleibt unberuehrt." + C.RESET)
        print(C.DIM + "   Bleibt auch nach Reboot aus. Wieder aktivieren: "
              "sudo rfkill unblock wlan bluetooth" + C.RESET)
    else:
        if args.mock:
...
def check_swap(args):
    ...
    def swap_aktiv():
        try:
            ...
            return False
    if not swap_aktiv():
        print(C.GRN + " \u2714 Kein aktiver Swap \u2014 RAM-Daten koennen nicht auf die "
              "SD-Karte auslaufen." + C.RESET)
        return
    import subprocess
    try:
...
def secure_cleanup(args):
    ...
    hr()
    print(C.BOLD + " Aufraeumen (keine Spuren)" + C.RESET)
    print(C.DIM + "   Hinweis: Alle Entropie- und Seed-Daten wurden ausschliesslich "
          "im RAM verarbeitet;" + C.RESET)
    print(C.DIM + "   dieses Skript hat keine Datei geschrieben." + C.RESET)
    if args.yes:
    ...
    try:
        antwort = input("   Bash-Verlaufsdateien (.bash_history) zusaetzlich leeren? "
                        "[ja/nein]: ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        antwort = "nein"
    if antwort == "ja":
...
def main():
    ...
    print(C.RED + C.BOLD + "  WICHTIG:" + C.RESET + C.RED +
          ...
          "  auf einem zweiten, unabhaengigen Offline-Geraet verifizieren." + C.RESET)
    secure_cleanup(args)
