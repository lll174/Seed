...
ASCII_DEKO = r"""      ☢ Radiozerfall ─┐  ┌─ ⌁ HWRNG   ┌─ ⌂ IR-Kamera   ┌─ ⚄ Wuerfel
                      └──┴────────────┴────────────────┘
...
def banner():
    ...
    if w >= 84:
        ...
        print(C.DIM + ASCII_DEKO + C.RESET)
        print()
        t = f"QUANTUM SEED GENERATOR v{VERSION} · Hardware-Entropie · Raspberry Pi 5"
        print(C.BOLD + t.center(w) + C.RESET)
    else:
        # Fallback fuer schmale Terminals
        line = "═" * (w - 2)
        print(C.ORA + "╔" + line + "╗" + C.RESET)
        t = f"QUANTUM SEED GENERATOR v{VERSION}  ·  Hardware-Entropie  ·  BIP39"
        print(C.ORA + "║" + C.BOLD + t.center(w - 2)[:w-2] + C.RESET + C.ORA + "║" + C.RESET)
        print(C.ORA + "╚" + line + "╝" + C.RESET)
    ...
    if methode:
        ...
        for typ in ("wlan", "bluetooth"):
            try:
                ...
                pass
        status = f" (verifiziert: {', '.join(geprueft)})" if geprueft else ""
        ...
        antwort = input(t("   Trotzdem fortfahren? Nur 'ja' setzt fort: ", "   Continue anyway? Only 'ja' continues: ")).strip().lower()
        if antwort != "ja":
...
def secure_cleanup(args):
    ...
    if args.yes:
        print(C.YEL + "   (--yes: Bildschirm wird nicht automatisch geloescht \u2014 "
              "vor dem Verlassen" + C.RESET)
        print(C.YEL + "    'clear' ausfuehren und das Terminalfenster schliessen; "
              "optional: history -c)" + C.RESET)
        return
    try:
        input(C.BOLD + "   Seed sicher auf Papier notiert? [Enter] loescht jetzt "
              "die komplette Bildschirmanzeige \u2026 " + C.RESET)
    except (EOFError, KeyboardInterrupt):
        pass
    # Bildschirm + Scrollback-Puffer des Terminals loeschen
    print("\033[3J\033[2J\033[H", end="", flush=True)
    print(C.GRN + " \u2714 Bildschirmanzeige und Scrollback geloescht." + C.RESET)
    try:
        antwort = input(t("   Bash-Verlaufsdateien (.bash_history) zusaetzlich leeren? [ja/nein]: ", "   Also clear bash history files (.bash_history)? [ja/nein]: ")).strip().lower()
    except (EOFError, KeyboardInterrupt):
        antwort = "nein"
    if antwort == "ja":
        ...
        for p in kandidaten:
            try:
                if os.path.exists(p):
                    open(p, "w").close()
                    print(C.GRN + f"   \u2714 geleert: {p}" + C.RESET)
            except Exception as e:
                print(C.YEL + f"   \u26a0 {p}: {e}" + C.RESET)
        print(C.DIM + "   Hinweis: Die laufende Shell haelt ihren Verlauf im RAM \u2014 "
              "dieses Terminalfenster" + C.RESET)
        print(C.DIM + "   jetzt SCHLIESSEN (oder 'history -c' ausfuehren), damit "
              "nichts zurueckgeschrieben wird." + C.RESET)
    else:
        print(C.DIM + "   Verlaufsdateien unveraendert. Empfehlung: Terminalfenster "
              "schliessen." + C.RESET)
...
def configure(args):
    ...
    words = 24 if ask_int(t("Seed-Laenge: 12 oder 24 Woerter", "Seed length: 12 or 24 words"), args.words, 12, 24) >= 18 else 12
    ...
    decay  = ask_int(t(f"Zerfallsereignisse (Minimum {mind}, Empf. \u2265{max(512, 2*target)}; ~17 CPM \u21d2 {max(512,2*target)/17:.0f} min)", f"Decay events (minimum {mind}, recomm. \u2265{max(512, 2*target)}; ~17 CPM \u21d2 {max(512,2*target)/17:.0f} min)"),
                     max(args.decay, mind), mind, 100000)

    frames = ask_int(t("Kamera-Frames MLX90640 (Empf. ≥16)", "Camera frames MLX90640 (recomm. ≥16)"), args.frames, 2, 1000)
...
def main():
    ...
    print(C.DIM + t("   (Dauer haengt von der Zaehlrate ab — bei Hintergrundstrahlung typ. 30-120 min fuer die empfohlene Ereigniszahl)", "   (Duration depends on count rate — with background radiation typ. 30-120 min for the recommended event count)") + C.RESET)
