...
def play_success_jingle():
    ...
    try:
        ...
        if not _shutil.which("aplay"):
            return
        rate = 22050
        # ~1.8 s Retro-Sequenz in drei Akten:
        #   1. schneller Power-Up-Lauf (Sechzehntel aufwaerts)
        #   2. Fanfare C-E-G mit Punch
        #   3. hohes C mit 8-bit-Vibrato als Schlussakkordton
        C5, D5, E5, G5, A5, C6, E6, G6 = (523.25, 587.33, 659.25, 783.99,
                                          880.00, 1046.50, 1318.51, 1567.98)
        noten = [
            (C5, 0.07, 0.0), (D5, 0.07, 0.0), (E5, 0.07, 0.0), (G5, 0.07, 0.0),
            (A5, 0.07, 0.0), (C6, 0.07, 0.0), (E6, 0.07, 0.0), (G6, 0.12, 0.0),
            (C5, 0.16, 0.0), (E5, 0.16, 0.0), (G5, 0.16, 0.0),
            (C6, 0.60, 6.0),                      # Schlusston mit Vibrato (6 Hz)
        ]
        frames = bytearray()
        for freq, dauer, vibr in noten:
            n = int(rate * dauer)
            for i in range(n):
                f = freq
                if vibr > 0:
                    f = freq * (1.0 + 0.012 * math.sin(2 * math.pi * vibr * i / rate))
                periode = rate / f
                pegel = 0.30 * (1.0 - i / n * 0.30)
                wert = pegel if (i % periode) < (periode / 2) else -pegel
                frames += struct.pack("<h", int(wert * 32767))
            frames += b"\x00\x00" * int(rate * 0.012)   # Staccato-Luecke
        buf = _io.BytesIO()
...
def verify_seed_transcription(idx, words_list, args):
    ...
    if args.yes or args.mock or not words_list or not sys.stdin.isatty():
        return
    try:
        antwort = input(t("   Notierte Woerter zur Kontrolle erneut eingeben? [ja/nein]: ",
                          "   Re-enter the written words for verification? [yes/no]: ")).strip().lower()
    except (EOFError, KeyboardInterrupt):
        return
    if antwort not in ("ja", "yes", "y"):
        return
    print(C.DIM + t("   Alle Woerter in einer Zeile (Leerzeichen und/oder Kommas als Trenner):",
                    "   All words on one line (spaces and/or commas as separators):") + C.RESET)
    try:
        roh = input("   > ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        return
...
def configure(args, cpm_mess=None):
    ...
    if args.yes:
        ...
        if args.frames < 2:
            fail_abort("Konfiguration",
                       t(f"--frames {args.frames} < 2: Der Identische-Frames-Check "
                         "braucht mindestens 2 Frames (Audit M5).",
                         f"--frames {args.frames} < 2: the identical-frames check "
                         "requires at least 2 frames (audit M5)."))
...
def main():
    ...
    if not (2 <= args.frames <= 1000):
        ap.error("--frames ausserhalb 2..1000")
