...
CAM_MIN_DIFF          = 0.51  # Frame nur akzeptieren, wenn >=51% Pixel anders
...
def collect_camera(n_frames: int, mock: bool, transparent: bool = False) -> SourceResult:
    frames = []
    rejected = 0
    print(C.YEL + C.BOLD + "   >>> Bitte jetzt die Hand in ca. 60 cm Abstand ZUFAELLIG "
          "vor der Kamera bewegen! <<<" + C.RESET)
    print(C.DIM + f"   (Frames werden nur akzeptiert, wenn sich >= {CAM_MIN_DIFF*100:.0f}% "
          "der Pixel zum letzten Frame unterscheiden)" + C.RESET)
    if mock:
        for i in range(n_frames):
            ...
            if transparent:
                ...
                print(f"   Frame {i+1:3d}/{n_frames}: SHA256 {h[:16]}…  Diff {d*100:5.1f}%  "
        ...
        try:
            ...
            while len(frames) < n_frames:
                ...
                if frames and d < CAM_MIN_DIFF:
                    ...
                    consec_reject += 1
                    if transparent:
                        print(C.DIM + f"   verworfen: Diff nur {d*100:5.1f}% "
                              f"(< {CAM_MIN_DIFF*100:.0f}%) — Hand bewegen!" + C.RESET)
                    if consec_reject >= 200:
                        fail_abort("MLX90640",
                                   "200 Frames in Folge unter der 51%-Schwelle — "
                                   "Sensorbild erstarrt (Waermequelle statisch / Sensor defekt?).")
                    continue
                consec_reject = 0
                frames.append(frame)
                if transparent:
                    h = hashlib.sha256(frame).hexdigest()
                    print(f"   Frame {len(frames):3d}/{n_frames}: SHA256 {h[:16]}…  "
