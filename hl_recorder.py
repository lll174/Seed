...
-- Offene Orders der Wale, vor allem Trigger-Orders (Stop-Loss, Take-Profit).
-- Die stehen NICHT im Orderbuch: Sie werden erst beim Auslösepreis zu
-- Market-Orders -- wie Liquidationen. Ein Stop-Cluster ist deshalb ein
-- Kaskaden-Kandidat. Ein Datensatz je Order und Abfrage (Zeitreihe).
CREATE TABLE IF NOT EXISTS trigger_orders (
    ts          INTEGER,
    addr        TEXT,
    coin        TEXT,
    oid         INTEGER,
    side        TEXT,       -- A = Verkauf, B = Kauf
    order_type  TEXT,       -- Stop Market, Stop Limit, Take Profit Market, Take Profit Limit, Limit
    is_trigger  INTEGER,
    trigger_px  REAL,
    limit_px    REAL,
    size        REAL,
    notional    REAL,       -- Größe * Auslösepreis (bzw. Limitpreis)
    reduce_only INTEGER,
    is_tpsl     INTEGER,    -- an eine Position gebundener Stop/TP
    kind        TEXT,       -- stop_sell / stop_buy / tp_sell / tp_buy / limit_sell / limit_buy
    PRIMARY KEY (ts, oid)
) WITHOUT ROWID;
CREATE INDEX IF NOT EXISTS idx_trig_coin_ts ON trigger_orders(coin, ts);
...
HL_WEIGHT_BUDGET = 1200          # Hyperliquid: Gewichtseinheiten je Minute und IP
WEIGHT_STATE = 2                 # clearinghouseState, l2Book, allMids ...
WEIGHT_OTHER = 20                # frontendOpenOrders, meta und der Rest


class RateLimiter:
    """
    Token-Bucket in GEWICHT je Minute, nicht in Requests: Eine Positionsabfrage
    kostet 2, eine Order-Abfrage 20. Dazu ein 60-s-Fenster, das die tatsächlich
    verbrauchten Einheiten zählt -- das ist die Zahl, die das Dashboard zeigt.
    """

    def __init__(self, weight_per_min: int):
        self.rate = weight_per_min / 60.0
        self.capacity = max(weight_per_min / 6.0, 20.0)
        self.tokens = self.capacity
        self.last = time.monotonic()
        self._lock = asyncio.Lock()
        self.window: deque = deque()          # (Zeit, Gewicht) der letzten 60 s

    async def acquire(self, weight: int = WEIGHT_STATE) -> None:
...
class Settings:
    ...
    # Stop-/Trigger-Orders der Wale
    stop_interval: int = 120        # Sekunden zwischen zwei Order-Abfragen je Wal (>= 20 Mio)
    stop_min_usd: float = 1e6       # Konten ab diesem Positionswert werden nach Orders gefragt
    stop_interval_big: int = 600    # Mindestabstand für Konten unter Wal-Größe
    stop_weight_budget: int = 300   # Gewichtseinheiten/min für Order-Abfragen (15 Abfragen)
...
class Recorder:
    def __init__(self, con: sqlite3.Connection, cfg: Settings):
        self.con = con
        self.cfg = cfg
        # Budget: Positionsabfragen (Gewicht 2) plus Wal-Orders (Gewicht 20) plus
        # Reserve, gedeckelt bei 85 % des Hyperliquid-Limits
        self.limiter = RateLimiter(min(int(HL_WEIGHT_BUDGET * 0.85),
                                       cfg.polls_per_min * WEIGHT_STATE + cfg.stop_weight_budget + 60))
        ...
        self.stop_polled_at: dict[str, float] = {}   # Wal-Adresse -> letzte Order-Abfrage
    ...
    async def post(self, payload: dict, weighted: bool = True,
                   weight: int = WEIGHT_STATE) -> dict | list | None:
        import aiohttp
        if weighted:
    ...
    def write_snapshot(self) -> None:
        ...
        self.con.execute("INSERT OR REPLACE INTO meta VALUES ('api_weight_1m', ?)",
                         (str(self.limiter.used_last_minute()),))
        self.con.execute("INSERT OR REPLACE INTO meta VALUES ('api_weight_budget', ?)",
                         (str(HL_WEIGHT_BUDGET),))
        self.con.execute("INSERT OR REPLACE INTO meta VALUES ('api_weight_ts', ?)", (str(now),))
        lag = self.poll_lag()
        self.con.execute(
    ...
    async def run_flush(self) -> None:
        ...
        while not self.stop.is_set():
            ...
            try:
                ...
                self.con.execute("INSERT OR REPLACE INTO meta VALUES ('api_weight_1m', ?)",
                                 (str(self.limiter.used_last_minute()),))
                self.con.execute("INSERT OR REPLACE INTO meta VALUES ('api_weight_ts', ?)",
                                 (str(int(time.time())),))
                self.con.commit()
    ...
    @staticmethod
    def classify_order(o: dict, mark: float) -> tuple[str, float]:
        """Art der Order und maßgeblicher Preis für die Kaskaden-Frage."""
        side = o.get("side")
        trig = o.get("isTrigger") or o.get("triggerPx") not in (None, "0.0", "0", 0)
        typ = str(o.get("orderType") or ("Limit"))
        tpx = float(o.get("triggerPx") or 0) or float(o.get("limitPx") or 0)
        if trig and tpx:
            if typ.startswith("Stop"):
                return ("stop_sell" if side == "A" else "stop_buy"), tpx
            if typ.startswith("Take"):
                return ("tp_sell" if side == "A" else "tp_buy"), tpx
            # unbenannter Trigger: Lage zum Kurs entscheidet
            if side == "A":
                return ("stop_sell" if tpx < mark else "tp_sell"), tpx
            return ("stop_buy" if tpx > mark else "tp_buy"), tpx
        return ("limit_sell" if side == "A" else "limit_buy"), tpx

    async def poll_whale_orders(self, addr: str) -> None:
        """
        frontendOpenOrders für eine Wal-Adresse: Gewicht 20, zehnmal so teuer wie
        eine Positionsabfrage -- deshalb nur Wale, nur alle stop_interval Sekunden.
        Auf HIP-3-Dexen gesondert abfragen, sonst fehlen die Orders dort.
        """
        dexes = [None] + sorted(self.addr_dex.get(addr, set()) - {""})
        rows, now = [], int(time.time())
        for dex in dexes:
            payload = {"type": "frontendOpenOrders", "user": addr}
            if dex:
                payload["dex"] = dex
            data = await self.post(payload, weight=WEIGHT_OTHER)
            if not isinstance(data, list):
                continue
            for o in data:
                sym = str(o.get("coin", ""))
                coin = self.coin_of.get(sym) or sym.split(":")[-1]
                if coin not in self.symbol_of:
                    continue
                mark = self.ref_px(coin) or 0.0
                kind, px = self.classify_order(o, mark)
                try:
                ...
                if not px or sz <= 0:
                    continue
                rows.append((now, addr, coin, int(o.get("oid") or 0), o.get("side"),
                             str(o.get("orderType") or "Limit"),
                             int(bool(o.get("isTrigger"))),
                             float(o.get("triggerPx") or 0), float(o.get("limitPx") or 0),
                             sz, sz * px, int(bool(o.get("reduceOnly"))),
                             int(bool(o.get("isPositionTpsl"))), kind))
        self.stop_polled_at[addr] = time.time()
        if rows:
            self.con.executemany("INSERT OR REPLACE INTO trigger_orders VALUES "
                                 "(?,?,?,?,?,?,?,?,?,?,?,?,?,?)", rows)
            self.stats["order_rows"] += len(rows)
        # Auch ein leeres Ergebnis ist eine Information: Marker-Zeile ohne Order
        else:
            self.con.execute("INSERT OR REPLACE INTO trigger_orders VALUES "
                             "(?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                             (now, addr, "", 0, "", "none", 0, 0, 0, 0, 0, 0, 0, "none"))

    def order_candidates(self) -> list[str]:
        """
        Wer wird nach Orders gefragt, in dieser Reihenfolge:
          1. Wale (>= 20 Mio), fällig alle stop_interval Sekunden
          2. Konten ab stop_min_usd Positionswert, fällig alle stop_interval_big
             Sekunden, die am längsten nicht abgefragten zuerst
        Das Budget (stop_weight_budget je Minute) begrenzt, wie viele davon
        je Runde drankommen -- Wale werden dadurch nie von Großkonten verdrängt.
        """
        now = time.time()
        whales = set(self.whale_state.keys())
        due = [a for a in sorted(whales, key=lambda a: self.stop_polled_at.get(a, 0))
               if now - self.stop_polled_at.get(a, 0) >= self.cfg.stop_interval]
        big = []
        for addr, positions in self.positions.items():
            if addr in whales:
                continue
            if sum(p.get("pos_value", 0) for p in positions) >= self.cfg.stop_min_usd:
                if now - self.stop_polled_at.get(addr, 0) >= self.cfg.stop_interval_big:
                    big.append(addr)
        big.sort(key=lambda a: self.stop_polled_at.get(a, 0))
        self.stats["order_candidates"] = len(whales) + len(big)
        return due + big

    async def run_stop_orders(self) -> None:
        """Wale und Großkonten reihum nach ihren Orders fragen, im Budget stop_weight_budget."""
        await asyncio.sleep(90)
        per_round = max(1, self.cfg.stop_weight_budget // WEIGHT_OTHER)   # je Minute
        while not self.stop.is_set():
            try:
                due = self.order_candidates()
                for addr in due[:per_round]:
                    await self.poll_whale_orders(addr)
                    if self.stop.is_set():
                        return
            except Exception as e:
                print(f"  Wal-Orders: {e}")
    ...
    async def run_snapshots(self) -> None:
        while not self.stop.is_set():
            ...
            print(f"[{time.strftime('%H:%M:%S')}] Adressen {n_addr:>6} | "
                  ...
                  + f" | API {self.limiter.used_last_minute()}/{HL_WEIGHT_BUDGET}"
                  + (f" | Wal-Orders {self.stats['order_rows']}"
                     if self.stats['order_rows'] else ""))
        ...
        async with aiohttp.ClientSession(connector=connector) as s:
            ...
            tasks = [asyncio.create_task(t) for t in
                     (self.run_ws(), self.run_poller(), self.run_snapshots(),
                      self.run_flush(), self.run_backup(), self.run_stop_orders())]
...
def do_backup(db_path: str, backup_dir: str) -> dict:
    ...
    try:
        ...
        # In EINEM Schritt kopieren. Schrittweise mit Pausen klingt schonender,
        # führt aber in eine Endlosschleife: Verändert eine andere Verbindung die
        # Quelle zwischen zwei Schritten -- der Recorder schreibt alle 10 s --,
        # beginnt die Backup-API von vorn. Ein einzelner Schritt läuft in einer
        # Lesetransaktion und wird von Commits nicht unterbrochen (WAL-Modus:
        # Leser blockieren Schreiber nicht). Die Geschwindigkeit begrenzt dann
        # das Zielmedium, typisch 10-30 MB/s auf einer SD-Karte.
...
def audit(con: sqlite3.Connection) -> bool:
    ...
    # --- Wal-Orders
    if has("trigger_orders"):
        r = con.execute("""SELECT COUNT(DISTINCT addr), SUM(kind LIKE 'stop_%'), SUM(kind LIKE 'tp_%'),
                                  MAX(ts) FROM trigger_orders WHERE ts > ?""", (now - 1200,)).fetchone()
        if r[0]:
            line(OK, "Wal-Orders", f"{r[0]} Wale in den letzten 20 min, {r[1] or 0} Stops, "
                                   f"{r[2] or 0} Take-Profits, jüngste vor {(now - r[3])//60} min")
        else:
            line(WARN, "Wal-Orders", "keine Abfrage in den letzten 20 min — Recorder älter als 2 min?")
    api = dict(con.execute("SELECT k, v FROM meta WHERE k IN ('api_weight_1m','api_weight_budget')"))
    if "api_weight_1m" in api:
        used, budget = int(api["api_weight_1m"]), int(api.get("api_weight_budget", 1200))
        line(OK if used < budget * 0.8 else WARN, "API-Budget",
             f"{used} von {budget} Gewicht/min ({used / budget * 100:.0f} %)")
...
def main() -> None:
    ...
    p.add_argument("--stop-interval", type=int, default=120,
                   help="Sekunden zwischen zwei Order-Abfragen je Wal (Gewicht 20)")
    p.add_argument("--stop-min-usd", type=float, default=1e6,
                   help="Konten ab diesem Positionswert nach Orders fragen (Wale immer)")
    p.add_argument("--stop-budget", type=int, default=300,
                   help="Gewicht/min für Wal-Order-Abfragen (0 = aus)")
    ...
    cfg = Settings(coins=args.coins, bucket_pct=args.bucket_pct,
                   ...
                   stop_interval=args.stop_interval, stop_weight_budget=args.stop_budget,
                   stop_min_usd=args.stop_min_usd)
    con.execute("INSERT OR REPLACE INTO meta VALUES ('backup_dir', ?)", (backup_dir,))
    con.commit()
    if same_disk:
