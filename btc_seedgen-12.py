    ...
    if mock:
        ...
        for i in range(n_events):
            ...
            if transparent:
                ...
                print(f"   Ereignis {i+1:4d}/{n_events}: t = {ts_val} ns   Δ = {d:8.3f} s [MOCK]")
...
def secure_cleanup(args):
    ...
    if args.yes:
        print(C.YEL + t("   (--yes: Bildschirm wird nicht automatisch geloescht — vor dem Verlassen", "   (--yes: screen is not cleared automatically — before leaving,") + C.RESET)
        print(C.YEL + t("    'clear' ausfuehren und das Terminalfenster schliessen; optional: history -c)", "    run 'clear' and close the terminal window; optionally: history -c)") + C.RESET)
        return
    try:
