...
def play_success_jingle():
    ...
    try:
        ...
        if not _shutil.which("aplay"):
            return
        rate = 22050
        # Klassische Power-Up-Fanfare: C5 E5 G5 C6, letzter Ton laenger
        noten = [(523.25, 0.09), (659.25, 0.09), (783.99, 0.09), (1046.50, 0.28)]
        frames = bytearray()
        for freq, dauer in noten:
            n = int(rate * dauer)
            periode = rate / freq
            for i in range(n):
                # Rechteckwelle (50% Duty) + kurze Abkling-Huellkurve
