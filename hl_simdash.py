...
from hl_candles import CandleEngine, choose, discover
...
def chart_data(coin: str, t_from: int, t_to: int, agg_min: int) -> dict:
    conn = ro_conn()
    step = agg_min * 60
    cur = conn.execute(
    ...
    for r in cur.fetchall():
        ...
        b = buckets.get(k)
        if b is None:
            buckets[k] = {"ts": k, "o": r["open"], "h": r["high"],
    ...
    def do_GET(self):
        ...
        try:
            ...
            if u.path == "/api/chart":
                ...
                return self._send(chart_data(coin, t_from, t_to, agg))
...
<section>
  <div class="row">
    ...
    <select id="span">
      ...
      <option value="72">3 Tage</option><option value="168">7 Tage</option>
    </select>
...
// ---------- Chart ----------
let CURSOR = null, POS = {}, loading = false;
...
async function loadChart(anchor){
  ...
  try {
    ...
    const r = await fetch(
      `/api/chart?from=${to - hours*3600}&to=${to}&agg=${agg}`);
    CHART = await r.json();
    draw();
...
function draw(){
  ...
  if(!CHART || !CHART.candles || !CHART.candles.length){
    ...
    ctx.fillText('Keine Kursdaten im gewählten Bereich.', 20, 40); return; }
